...
import { IBookingPopulated } from '../types/booking';
import fs from 'fs';
import path from 'path';

export const generateInvoicePDF = (res: Response, booking: IBookingPopulated) => {
  try {
    ...
    doc
      ...
      .text(`Phone: ${booking.userDetails?.phoneNumber || 'N/A'}`, 50, startY + 55)
      .text(`Address: ${booking.userDetails?.address || 'N/A'}`, 50, startY + 70);
    ...
    const addWrappedTableRow = (label: string, value: string) => {
      const rowHeight = 25;
